// =============================================================================
// Workload Identity Defense (WID) Platform — Shared Data Plane Core
// =============================================================================
//
...
class RateLimiter {
  ...
  /** Returns remaining requests for a principal in the current window. */
  getRemaining(principal) {
  ...
  /** Returns rate limit headers object for a principal. */
  getRateLimitHeaders(principal) {
    const key = principal || 'unknown';
    const window = this.windows.get(key);
    const now = Date.now();
    if (!window || now > window.resetAt) {
  ...
  /** Starts periodic cleanup of expired windows. */
...
# Workload Identity Defense (WID) Platform — iptables transparent redirect
...
const { TLSManager, FederationCA } = require('./tls-manager');
