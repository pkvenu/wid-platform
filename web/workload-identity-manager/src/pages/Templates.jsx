...
const FIELDS = [
  // ── Workload-level fields (compliance, enforcement, lifecycle policies) ──
  ...
  { key: 'type',           label: 'Identity Type',   type: 'select', options: ['a2a-agent','mcp-server','lambda','container','iam-role','iam-user','service-account','api-key','secret','cloud-run-service'] },
  ...
  // ── Access policy fields (client.* / server.* for gateway evaluation) ──
  { key: 'client.type',       label: 'Source Type',        type: 'select', options: ['a2a-agent','mcp-server','service','cloud-run-service','user'] },
  { key: 'client.is_ai_agent',label: 'Source Is AI Agent', type: 'boolean' },
  { key: 'client.name',       label: 'Source Name',        type: 'string' },
  { key: 'client.trust_level',label: 'Source Trust Level', type: 'select', options: ['cryptographic','very-high','high','medium','low','none'] },
  { key: 'client.verified',   label: 'Source Attested',    type: 'boolean' },
  { key: 'server.type',       label: 'Dest Type',          type: 'select', options: ['a2a-agent','mcp-server','service','external-api','external-credential'] },
  { key: 'server.name',       label: 'Dest Name',          type: 'string' },
  { key: 'server.category',   label: 'Dest Category',      type: 'string' },
  { key: 'runtime.method',    label: 'HTTP Method',        type: 'select', options: ['GET','POST','PUT','DELETE','PATCH'] },
  { key: 'runtime.path',      label: 'Request Path',       type: 'string' },
];
...
const ACTIONS = [
  { key:'flag', label:'Flag violation' },
  { key:'block_deploy', label:'Block deployment' },
  { key:'require_attest', label:'Require re-attestation' },
  { key:'revoke_access', label:'Revoke access' },
  { key:'notify', label:'Notify owner/team' },
  { key:'quarantine', label:'Quarantine' },
  { key:'rotate_credential', label:'Rotate credential' },
  { key:'block', label:'Block request' },
...
function TemplateEditor({ template, onClose, onSave }) {
  ...
  const updateCondition = (i, k, v) => {
    const updated = form.conditions.map((c, j) => j === i ? { ...c, [k]: v } : c);
    update('conditions', updated);
  };

  const addAction = () => update('actions', [...form.actions, { type: 'flag', message: '' }]);
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            ...
            {form.conditions.map((c, i) => {
              const fDef = FIELDS.find(f => f.key === c.field) || FIELDS[0];
              const ops = OPS[fDef.type] || OPS.string;
              const needsVal = !['is_true','is_false','exists','not_exists'].includes(c.operator);
              return (
                <div key={i} className="flex items-center gap-2 mb-2">
                  <span className="w-5 h-5 rounded-full bg-white/[0.04] border border-white/[0.06] flex items-center justify-center text-[9px] text-nhi-dim font-bold shrink-0">{i+1}</span>
                  <select value={c.field} onChange={e => {
                    const f = FIELDS.find(fi => fi.key === e.target.value) || FIELDS[0];
                    const fieldOps = OPS[f.type] || OPS.string;
                    updateCondition(i, 'field', e.target.value);
                    updateCondition(i, 'operator', fieldOps[0].v);
                    updateCondition(i, 'value', '');
                  }} className="text-[10px] bg-surface-2 border border-[var(--border)] rounded px-2 py-1.5 text-nhi-text w-40">
                    <optgroup label="Workload Fields">
                      {FIELDS.filter(f => !f.key.includes('.')).map(f => <option key={f.key} value={f.key}>{f.label}</option>)}
                    </optgroup>
                    <optgroup label="Access Policy (Source)">
                      {FIELDS.filter(f => f.key.startsWith('client.')).map(f => <option key={f.key} value={f.key}>{f.label}</option>)}
                    </optgroup>
                    <optgroup label="Access Policy (Destination)">
                      {FIELDS.filter(f => f.key.startsWith('server.')).map(f => <option key={f.key} value={f.key}>{f.label}</option>)}
                    </optgroup>
                    <optgroup label="Runtime">
                      {FIELDS.filter(f => f.key.startsWith('runtime.')).map(f => <option key={f.key} value={f.key}>{f.label}</option>)}
                    </optgroup>
                  </select>
                  ...
                  {needsVal && (fDef.options
                    ...
                    : <input type={fDef.type === 'number' ? 'number' : 'text'} value={c.value || ''} onChange={e => updateCondition(i, 'value', e.target.value)}
                        placeholder={fDef.type === 'date' ? 'days' : fDef.type === 'string' ? 'value or regex...' : 'value'}
                        className="text-[10px] bg-surface-2 border border-[var(--border)] rounded px-2 py-1.5 text-nhi-text flex-1 focus:border-accent focus:outline-none" />
                  )}
            ...
            {form.actions.map((a, i) => (
              <div key={i} className="flex items-center gap-2 mb-2">
                ...
                <select value={a.type} onChange={e => updateAction(i, 'type', e.target.value)}
                  className="text-[10px] bg-surface-2 border border-[var(--border)] rounded px-2 py-1.5 text-nhi-text w-40">
                  {ACTIONS.map(ac => <option key={ac.key} value={ac.key}>{ac.label}</option>)}
                </select>
